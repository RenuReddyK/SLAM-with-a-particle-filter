...
def run_slam(src_dir, log_dir, idx, split):
    ...
    while(abs(slam.lidar[t0]['t'] < float(slam.joint['t'][0]))):
        t0 += 1
    xyth_i = slam.lidar[t0]['xyth']
    xyth_i[2] = slam.lidar[t0]['rpy'][2]
    slam.init_particles(n=1,p=xyth_i.reshape((3,1)),w=np.array([1]))
    slam.observation_step(t=0)
    num = 8
    weights = np.ones(num)/float(num)
    p = np.zeros((3,num), dtype=np.float64)
    slam.init_particles(num, p, weights)    
