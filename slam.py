...
class map_t:
    ...
    def grid_cell_from_xy(s, x, y):
        ...
        cell_x = (np.abs(np.clip(x, s.xmin, s.xmax) - s.xmin)/ s.resolution)
        cell_y = (np.abs(np.clip(y, s.ymin, s.ymax) - s.ymin)/ s.resolution)
        cell_grid = np.vstack((cell_x, cell_y)).T
        cell_grid =  np.array(cell_grid, dtype = int)
        return cell_grid
...
class slam_t:
    ...
    def stratified_resampling(p, w):
        ...
        for i in range(n):
            wei = np.random.uniform(0, 1/n) + (i-1)/n
            while (wei > x):
                count = count + 1
                x = x+ w[count]
            new_w[i] = new_w[count]
    ...
    def rays2world(s, p, d, head_angle=0, neck_angle=0, angles=None):
        ...
        d = is an array that stores the distance of along the ray of the lidar, for each ray 
        ...
        distance = np.clip(d, s.lidar_dmin, s.lidar_dmax)

        T_lidar_to_body = euler_to_se3(0, head_angle, neck_angle, np.array([0, 0, 0.15]))
        T_body_to_world = euler_to_se3(0, 0, p[2], np.array([p[0], p[1], s.head_height]))
        Final = np.zeros((len(d),3))

        for i in range(len(distance)):
            # 1. from lidar distances to points in the LiDAR frame
            Lidar_x = distance[i] * np.cos(angles[i])
            Lidar_y = distance[i] * np.sin(angles[i])
            
            # 2. from LiDAR frame to the body frame
            Lidar_coordinates = np.array([Lidar_x, Lidar_y, 0]).reshape((3,1))
            Body_coordinates = np.matmul(T_lidar_to_body, make_homogeneous_coords_3d(Lidar_coordinates))
            
            # 3. from body frame to world frame
            World_coordinates = T_body_to_world @ Body_coordinates
            Final[i] = World_coordinates[:3,:].reshape((3,))
    ...
    def get_control(s, t):
        ...
        below at time t, to go to time t-1. need to use the smart_minus_2d function to get the 
        difference of the two poses and we will simply set this to be the control (delta x, delta y, delta theta)
        """
        ...
        #### TODO: XXXXXXXXXXX
        x_t = s.lidar[t]['xyth'][0]
        y_t = s.lidar[t]['xyth'][1]
        yaw_t =  s.lidar[t]['rpy'][2]
        x_t_minus_1 = s.lidar[t-1]['xyth'][0]
        y_t_minus_1 = s.lidar[t-1]['xyth'][1]
        yaw_t_minus_1 =  s.lidar[t-1]['rpy'][2]

        current_pose_t = np.array([x_t, y_t, yaw_t])
        previous_post_t_minus_1 = np.array([x_t_minus_1, y_t_minus_1, yaw_t_minus_1])
        
        control = smart_minus_2d(current_pose_t, previous_post_t_minus_1)
    ...
    def dynamics_step(s, t):
        """"
        Compute the control using get_control and perform that control on each particle to get the 
        updated locations of the particles in the particle filter, remember to add noise using the 
    ...
    def update_weights(w, obs_logp):
        ...
        """
        #### TODO: XXXXXXXXXXX
        p_ = np.log(w) + obs_logp
        q_ = slam_t.log_sum_exp(p_)
        new_weights = np.exp(p_ - q_)
    ...
    def observation_step(s, t):
        ...
        """
        #### TODO: XXXXXXXXXXX
        Lidar_t = s.find_joint_t_idx_from_lidar(s.lidar[t]['t'])
        Head_angle_t = s.joint['head_angles'][1][Lidar_t]
        Neck_angle_t = s.joint['head_angles'][0][Lidar_t]

        scan_len = s.lidar[t]['scan']
        obs_log_probs = np.zeros((np.array(s.p).shape[1],))
        world_grid = []
        for i in range(np.array(s.p).shape[1]):
            projections = s.rays2world(s.p[:,i], scan_len,  Head_angle_t, Neck_angle_t, s.lidar_angles)     #(b) continued
            grid_map = np.array(s.map.grid_cell_from_xy(projections[:,0], projections[:,1]), dtype = int)
            world_grid.append(grid_map)
            if t!=0:
                obs_log_probs[i] = np.sum(s.map.cells[grid_map[:,0], grid_map[:,1]])   
            else:
                for j in grid_map:
                    x_ = j[0]
                    y_ = j[1]
                    s.map.cells[x_,y_] = 1
                obs_log_probs[i] = np.sum(s.map.cells) 
            
        s.w = s.update_weights(s.w, obs_log_probs)
        mapp = world_grid[np.argmax(s.update_weights(s.w, obs_log_probs))]
        s.map.log_odds = s.map.log_odds + s.lidar_log_odds_free
        s.map.log_odds[mapp[:,0], mapp[:,1]] = s.map.log_odds[mapp[:,0], mapp[:,1]] + s.lidar_log_odds_occ - s.lidar_log_odds_free
